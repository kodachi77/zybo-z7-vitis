...
# Usage (No Defaults):
#   set argv "-r <root_path> -x <xpr_path> -v <vivado_version> -b -no-block"
#   source generate_project.tcl
# Usage (All Defaults):
#   set argv ""
#   source generate_project.tcl

# Handle root_path argument
set idx [lsearch ${argv} "-r"]
if {${idx} != -1} {
    set root_path [glob -nocomplain [file normalize [lindex ${argv} [expr {${idx}+1}]]]]
} else {
    # Default
...
if {${idx} != -1} {
    ...
    # Default
    set xpr_path [file join ${root_path} proj [file tail $root_path]].xpr]
}
...
puts "INFO: Capturing board information from $root_path/project_info.tcl"
...
set_property "ip_output_repo" "[file normalize "$root_path/proj/cache"]" $obj
...
puts "INFO: capturing IP-related settings from $root_path/project_info.tcl"
set_project_properties_pre_add_repo $proj_name

# Set IP repository paths
puts "INFO: Setting IP repository paths"
set obj [get_filesets sources_1]
set_property "ip_repo_paths" "[file normalize $root_path/repo]" $obj
...
add_files -quiet -norecurse [glob -nocomplain $root_path/src/hdl/*.v]
add_files -quiet -norecurse [glob -nocomplain $root_path/src/hdl/*.vhd]
add_files -quiet -norecurse [glob -nocomplain $root_path/src/hdl/*.sv]

# Add IPs
# TODO: handle IP core-container files
puts "INFO: Adding XCI IP sources"
add_files -quiet [glob -nocomplain $root_path/src/ip/*/*.xci]

# Add constraints
puts "INFO: Adding constraints"
add_files -quiet -norecurse -fileset constrs_1 [glob -nocomplain $root_path/src/constraints/*.xdc]

# Recreate block design
# TODO: handle multiple block designs
set ipi_tcl_files [glob -nocomplain "$root_path/src/bd/*.tcl"]
set ipi_bd_files [glob -nocomplain "$root_path/src/bd/*/*.bd"]
if {[llength $ipi_tcl_files] > 1} {
    # TODO: quit and log the error
    puts "ERROR: This script cannot handle projects containing more than one block design! More than one tcl script found in src/bd"
    ...
    add_files -norecurse -quiet -fileset sources_1 [glob -nocomplain $root_path/src/bd/*/*.bd]
    open_bd_design [glob -nocomplain $root_path/src/bd/*/*.bd]
    set design_name [get_bd_designs]
    set file "$root_path/src/bd/$design_name/$design_name.bd"
...
# Capture project-specific IP settings
puts "INFO: capturing run settings from $root_path/project_info.tcl"
set_project_properties_post_create_runs $proj_name

# Get the post_build script path
set post_build_script_path [file join ${root_path} post_build.tcl]
